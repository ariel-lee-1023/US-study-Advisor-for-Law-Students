...
**Source:** Siqi Chen, [Humanizer](https://github.com/blader/humanizer), version 2.11.2 (2025), MIT License; its diagnostic framework is based on Wikipedia's ["Signs of AI writing"](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing).
**Trigger:** load when drafting, rewriting, or line-editing prose that should sound like the applicant; when reviewing a draft for AI-writing patterns; or when the user asks to humanize text.
**What this file is:** a sentence-level editing and quality-control module. It removes recurring AI-writing patterns without changing claims or replacing the writer's voice. It does not supply admissions strategy, legal-writing doctrine, facts, or the advisor's reasoning temperament.

> **Precedence and boundary.** Preserve the user's requested meaning, facts, citations, and disclosure choices first. A genuine writing sample from the applicant controls voice and punctuation. The document's purpose, audience, scope, and stance come next. Use this module after those decisions are settled. Its pattern list is diagnostic, not an authorship detector: one pattern by itself does not prove AI use. For application materials, `references/personal-statements.md` controls genre and content strategy, while this module controls the prose-cleanup pass. For U.S. legal writing, `references/legal-writing.md` controls legal convention.
...
The user's request controls what you return. See [How to return the result](#how-to-return-the-result). Use the same rewrite process in every mode.
...
**Rewrite mode (default when the user asks for a rewrite).** Return the final rewrite and a short note about material changes. Do not make the user compare an unnecessary intermediate draft.

**Review mode.** When the user asks only for review, identify the patterns that materially weaken the text and prioritize the edits. Do not replace the whole draft unless the user also asks for a rewrite.
